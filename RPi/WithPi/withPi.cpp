...
int sendData(FILE* fp, char *ct, char *filename)
{
    ...
    sprintf(html, "<html><head><meta http-equiv=\"Content-Type\" " \
                  ...
                  "url=\"control.html\">" \
                  "<title>Raspberry Pi : Observation Server</title>" \
                  "</head><body><table>" \
                  "<tr align=\"center\"><img src=capture.bmp></tr>" \
    ...
    if(!strcmp(filename, "capture.bmp")) {
        ...
        int len, fd = open(filename, O_RDONLY); /* 파일을 연다. */
        do {
            len = read(fd, html, BUFSIZ); /* 파일을 읽어서 클라이언트로 보낸다. */
...
void PiWidget::monitor()
{
    ...
    getPressure(pressure_fd, &t_c, &pressure); /* 기압과 온도를 위한 함수 */
    getTemperature(temperature_fd, &temperature, &humidity); /* 온도와 습도를 위한 함수 */
